...
impl Firewall {
    ...
    fn print_layer(&self, layer: usize) -> Vec<String> {
        ...
        let tick = if self.tick >= self.delay { self.tick - self.delay } else { usize::max_value() };

        if depth == 0 {
            vec![header, if layer == tick {
                "(.)".to_string()
            } else {
                "...".to_string()
            }]
...
impl std::fmt::Display for Firewall {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        ...
        let v: Vec<Vec<String>> = (0..layer_cnt)
            .map(|l| self.print_layer(l))
            .collect();
        let v: Vec<String> = transpose(v).iter()
            .map(|l| l.join(" "))
            .collect();
        write!(f, "{}", v.join("\n"))
    }
...
fn main() {
    let input = stdin();
    let layers: BTreeMap<usize, usize> =
        input.lock().lines()
            .map(|l| {
