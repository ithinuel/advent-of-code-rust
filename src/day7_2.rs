...
impl Node {
    fn actual_weight(&self, tree: &BTreeMap<String, Node>) -> u32 {
        (if let Some(ref children) = self.children {
            let mut weights: Vec<(&String, u32)> = children.iter()
...
fn main() {
    ...
    let re = Regex::new(r"(?P<name>\w+) \((?P<weight>\d+)\)(?:\s*->\s*(?P<children>\w+(?:,\s*\w+)*))?").unwrap();
    let _ = input.read_to_string(&mut lines);
    for caps in re.captures_iter(&lines) {
        let name = caps["name"].to_string();
        let children: Option<Vec<String>> = if let Some(m) = caps.name("children") {
            Some(m.as_str()
                .split(", ")
                .map(|s| s.to_string())
                .collect())
        ...
        };
        {
            ...
            let node = tree.entry(name.clone())
                .or_insert_with(|| Node {
                    name: name.clone(),
                    weight: 0,
                    parent: None,
    ...
    let roots: Vec<&String> = tree.values()
    ...
    for root in roots {
        println!("{}.actual_weight: {}", root, tree[root].actual_weight(&tree)); 
    }
}
