...
fn main() {
    ...
    stdin().read_to_string(&mut key).expect("failed to read");
    let out: String = (0..128)
        .map(|i| {
            let mut key = key.trim().to_string();
            key.push_str(&format!("-{}", i));
            let mut seed: Vec<usize> = key.as_bytes().iter().map(|b| *b as usize).collect();
    ...
    let mut occupied: BTreeSet<(usize, usize)> = out
        .chars()
        .enumerate()
        .filter(|&(_, c)| c == '1')
        .map(|(i, _)| i.div_rem(&128))
        .collect();
